...
@bp.route("/html")
def html_editor_view():
    return render_template("html_editor.html")


@bp.route("/token/<path:token>")
def serve_live_preview(token):
    preview = live_preview.get_preview(token)
    if not preview:
        return (
            "<!doctype html><html><head><meta charset='utf-8'><title>Abgelaufen</title>"
            "<style>body{font-family:system-ui;display:flex;align-items:center;justify-content:center;"
            "height:100vh;margin:0;background:#1e1e1e;color:#d4d4d4;}</style></head>"
            "<body><div style='text-align:center'><h2>Preview nicht gefunden oder abgelaufen</h2>"
            "<p style='color:#858585'>Dieser Link ist nicht mehr aktiv.</p></div></body></html>",
            404,
            {"Content-Type": "text/html; charset=utf-8"},
        )
    return preview["html"], 200, {"Content-Type": "text/html; charset=utf-8"}


@bp.route("/api/preview/create", methods=["POST"])
def api_preview_create():
    data = request.json or {}
    result = live_preview.create_preview(
        html=data.get("html", ""),
...
@bp.route("/api/preview/list")
def api_preview_list():
    return jsonify({"ok": True, "previews": live_preview.list_previews()})


@bp.route("/api/preview/<path:token>", methods=["DELETE"])
def api_preview_delete(token):
    return jsonify({"ok": live_preview.delete_preview(token)})


@bp.route("/api/preview/<path:token>", methods=["PUT"])
def api_preview_update(token):
    data = request.json or {}
    ok = live_preview.update_preview(
        token=token,
        html=data.get("html", ""),
        title=data.get("title", ""),
    )
